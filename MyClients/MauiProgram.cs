		...
		using (var scope = app.Services.CreateScope())
		{
		    ...
		    if (!db.Grades.Any())
		    {
		        ...
		        db.Disciplines.AddRange(
		            new Discipline { Name = "Bouldering", Description = "Bouldering is a form of free climbing performed on small rock formations or artificial rock walls without ropes or harnesses." },
		            new Discipline { Name = "Top Rope Climbing", Description = "Top rope climbing is a style of climbing where the rope runs from a belayer up through an anchor at the top of the route and back down to the climber." },
		            new Discipline { Name = "Lead Climbing", Description = "Lead climbing is a technique where the climber clips the rope to protection points as they ascend the route." },
		            new Discipline { Name = "Speed Climbing", Description = "Speed climbing is a racing discipline where climbers race head-to-head on a standardized 15-meter wall." }
		        );
		        ...
		        db.Trainings.AddRange(
		            new Training { UserId = user.Id, TrainingDate = new DateOnly(2025, 6, 1), TrainingLog = "Good session, felt strong on the overhang." },
		            new Training { UserId = user.Id, TrainingDate = new DateOnly(2025, 6, 3), TrainingLog = "Worked on footwork and balance." },
		            new Training { UserId = user.Id, TrainingDate = new DateOnly(2025, 6, 5), TrainingLog = "Tough day, arms were pumped." }
		        );

		        db.PersonalRecords.AddRange(
		            new PersonalRecord { UserId = user.Id, DisciplineId = bouldering.Id, GradeId = grade7a.Id, RecordDate = new DateOnly(2025, 3, 10) },
		            new PersonalRecord { UserId = user.Id, DisciplineId = topRope.Id, GradeId = grade6c.Id, RecordDate = new DateOnly(2025, 4, 15) },
		            new PersonalRecord { UserId = user.Id, DisciplineId = lead.Id, GradeId = grade6b.Id, RecordDate = new DateOnly(2025, 5, 20) }
		        );
